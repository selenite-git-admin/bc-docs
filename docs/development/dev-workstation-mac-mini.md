...
- **No more manual relays.** Items that don't belong to a program's own exchange family (PR landings, docs/ADR PRs, small design questions, one-off reviews) go through the permanent `gen-` family, per ADR DEC-081931. Each message carries `Topic:`, `Kind:`, `Depth:` and `Re:` headers, and there is at most one open message per topic. Authority still comes only from the operator, told to Codex directly.
