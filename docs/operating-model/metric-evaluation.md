...
**Purpose.** The Metric Evaluator is the platform runtime component that applies one Metric Contract version to one evaluation target to produce one Metric Snapshot together with its governed proof surfaces — Evidence and, per the DEC-ebb3cd D-2 required contract, per-act Lineage (the mounted paths' as-built proof surfaces, including where Lineage is not written, are recorded in Evidence and Lineage at metric evaluation).

**Behavior.** At invocation, the Evaluator resolves the Metric Contract referenced by its evaluation target, interprets the Contract's `metric_binding` to determine the referenced Canonical Contract versions and Canonical Field names the formula consumes, evaluates whether the declared temporal gate is satisfied, gathers the Canonical Object versions required by the binding, applies the formula deterministically over the gathered Canonical Field values, classifies the result against the governed thresholds, emits the act's Metric Snapshots, and attempts proof at the same act. **Required contract (DEC-ebb3cd D-2):** one act-level Evidence, one per-evaluation-act Lineage (referencing the consumed Canonical Object versions and carrying the emitted Snapshot identities), and the Metric Evaluation Run. **As-built:** neither mounted primary path meets the Lineage contract in full — the governed orchestrator path writes the Evidence and the Run but **no Lineage**, and the legacy path writes one narrower `contract → metric_evaluation` Lineage. See Evidence and Lineage at metric evaluation.
...
| Lineage (required contract) | Per evaluation act | **DEC-ebb3cd D-2 requires** one per-act Lineage referencing the consumed Canonical Object versions and the applied Metric Contract version, carrying the emitted Snapshot identities through its reference set (per-act cardinality, not per-snapshot). This is the required contract; the mounted paths do not yet meet it in full — see the as-built note below. |

**As-built proof surfaces (both mounted primary paths).** Neither primary path currently writes the DEC-ebb3cd Lineage in full:

- The **governed orchestrator** path (`MetricEvaluationOrchestrator` → `GovernedMetricEvaluationService` → `GovernedMetricPersistenceAdapter` → `recordRunEvidence`) writes the act-level Evidence and the Run but calls no `createLineage` — it emits **no governed Lineage**. This is also the composite/secondary path.
- The **legacy** path (`MetricEvaluationController` → `MetricService.evaluateMetric` / `createGrainSnapshots`) writes one Lineage of the narrower shape `contract:<MC>:<version> → metric_evaluation:<id>` (relationship `evaluated_by`, `snapshotCount` in context) — it does **not** reference the consumed Canonical Object versions and does **not** carry per-snapshot identities.

The per-act Lineage with consumed/output identity surfaces is decided doctrine (DEC-ebb3cd D-2) and an outstanding platform gap, not current behavior. Readers that need per-snapshot provenance cannot yet rely on a Lineage reference set; where a Lineage record exists at all it is the narrow legacy shape. Run-level operational references exist for diagnostics; they do not stand in for the required per-act Lineage.
...
**E6 closure is therefore two-part, and does not change the upstream-selection policy:** (a) resolution must return the selected row's value **and** the exact `metric_evaluation_id` of the row it consumed — the current latest-accepted selection may remain, but the runtime must retain that exact consumed ID rather than only the Metric Contract UID; and (b) persist that ID in a real per-evaluation governed Lineage record alongside the emitted snapshot identity. Any change to the latest-accepted selection policy itself would require its own authority. Until both land, the composite path is an operative computation **without a governed proof chain** — its current upstream references must not be called fixed snapshots, and its state must not be called persisted or period-exact Lineage.
