...
The **Action Evaluator** is one of the three boundary machines under the Evaluator umbrella (Canonical, Metric, Action — DEC-0d5b39); each is an independently invoked Foundation boundary act, and the umbrella is a naming convenience that never collapses the boundaries. Action evaluation is the **fourth and final** boundary act, kept visible as a first-class boundary: the **Action Object is an authoritative progression object** (FND-3), not a consumption artifact — under the required contract it is emitted with its Evidence and per-object Lineage, and its set-once terminal-state attributes are recorded on the object itself under the immutability rule.

> **As-built: implementation absent (fail-closed).** Everything this chapter states about action-creation, outcome-resolution, Action Object emission, its Evidence/Lineage, and the Action Evaluation Run is the **Foundation-required contract**, not current behavior. The mounted Action write path is **disabled**: `ActionRepository.createActionObject` and `createActionEvaluation` throw `NotImplementedException` (D575), because they targeted the retired `envelope.action_object` / `envelope.action_evaluation` tables (dropped R5/D481) with no replacement. `progression.intervention_evaluation` exists but the Action repository/service is **not wired to it**. Because the write throws before any proof call, no Evidence or Lineage is emitted on a successful mounted creation path. The replacement persistence path is tracked (`task_9166a559`). And even behind mocks, the current dormant service shape **differs from this contract**: it creates a separate `action_evaluation` record rather than setting set-once terminal attributes on a preserved Action Object; its outcome Lineage is `action_object → metric_snapshot`, not a resolution-act-to-Intervention-Contract relationship; `forceCloseAction` emits Evidence but no Lineage; and no mounted code creates or finalizes an Action Evaluation Run. Read this chapter as the doctrine the replacement must satisfy, not as a description of live behavior — do not treat set-once Action Object updates, forced-closure Lineage, IC-reference Lineage, or an operative Action Evaluation Run as current until the selected persistence path demonstrates them.
