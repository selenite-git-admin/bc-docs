...
provenance: "Migrated verbatim from bc-core docs/design/platform-db-foundation-requirements.md, v0.7 at commit 2407b04b (retrievable historical revision), consolidated into bc-docs per the documentation-home policy. FR/NFR substance unchanged; only the §1.1 program-status re-anchor, the status metadata row and this front matter were reconciled to the landed state (d597 RESPONSE-148)."
errata_referenced: []
v2_sources: []
diagrams: []
---
...
| | |
|---|---|
| **Status** | Migrated into bc-docs (documentation-home policy) from bc-core `docs/design/platform-db-foundation-requirements.md` v0.7 (`2407b04b`, the retrievable historical revision). FR/NFR substance is unchanged from v0.7; only §1.1, this row and the front matter were reconciled to the landed state (d597 RESPONSE-148). **Current program status:** W1 complete and auditor-accepted for local development (2026-09-15, d597 RESPONSE-147). **W0 record work complete; coordination acknowledgement reported; independently verified W0 closure pending retained acknowledgement evidence** (the 2026-09-09 acknowledgement exists only as a conversation). Acceptance of this document ratifies no recovery objective (D10), baseline or wave design; those remain their own records. |
...
### 1.1 Program status — re-anchor 2026-09-15

This reconciles status against what has landed. It updates status only; **no requirement (FR/NFR) text below is changed** (d597 RESPONSE-148). Ground truth: `selenite-git-admin/bc-db` `main` at `6f103d9` (through PR #25). Gate ③ (baseline ratification) was crossed once under operator authorization; **gate ② was crossed once** for the single accepted local-development witnessed adoption (below); no gate ①/④ beyond the accepted W1.1 read-only capture has been crossed (no promotion).

**W0 — bind governing records.** D1 (mandate, DEC-75fc71), D2 (bc-docs PR #15 custody), D3 (`bc-db`, DEC-826390), D6 (BC-Agent deferral, DEC-66d3ca), D7 (launcher retired, DEC-3628b4), D8 (operating model, DEC-0e4547), D12 (capture authority = B, DEC-c99f72), D13 (CI engine pin) all ratified. **State: record work complete; coordination acknowledgement reported; independently verified W0 closure pending retained acknowledgement evidence (§13.4)** — the 2026-09-09 coordination acknowledgement exists only as a conversation and is not retained as an immutable record, so W0 does not independently close (d597 RESPONSE-148).
...
**W2–W5:** not started. Each still requires its own design, implementation, evidence and independent acceptance. W2 entry additionally rests on material design inputs — D4 (hosting-by-environment) and D5 (BYO-DB posture) — and on the tenant-onboarding design being committed at an immutable version with its ratification bound to a record (open question 8).
...
|---|---|---|---|
| **W0** | Bind governing records: study merged (done); this document merged; track DEC-591eb7 (bc-docs PR #15, merged); ratify the program mandate as a decision record (D1, done); formalize BC-Agent deferral (D6, done); coordinate with the Platform Readiness session (§13.4) | Records merged/ratified; coordination note acknowledged. **State:** record work complete (bc-docs PR #17 merged, `734574fa`); coordination acknowledgement **reported** (session message, 2026-09-09; §13.4); independently verified W0 closure **pending** retained acknowledgement evidence | Operator |
