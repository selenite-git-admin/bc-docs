...
- **The `Free` row is seed-drift** — present in neither canonical seed (`seed-v2/data/pricing/packages.csv`, `src/registry/seed/seed-packages.ts DEFAULT_PACKAGES` both emit the six tiers, no Free) → a from-zero rebuild would not reproduce it ([[TSK-ca2f7d]]).
