...
| L8 Metric Registry | `metric-directory.controller.ts:107` (@PlatformOnly, DEC-b5c7ff/D506) | `metric_directory` family=47/group=135/member=415/member_version=247/realization_event=263; `mcf.seed_metric`=12507 | **live-verified:** `catalog/metrics/directory` Metric Registry UI — tabbed Family/Group/Member (counts+coverage+realizability), member provenance detail, and governed create doors (New Family/Group direct + Author metric via M12 panel, live APPROVE_FOR_DRAFT run observed); bc-admin PR #45 | verified + live UI |
