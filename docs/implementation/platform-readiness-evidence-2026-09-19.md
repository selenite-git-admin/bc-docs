...
| L8 Metric Registry | `metric-directory.controller.ts:107` (@PlatformOnly, DEC-b5c7ff/D506) | `metric_directory` family=47/group=135/member=415/member_version=247/realization_event=263; `mcf.seed_metric`=12507 | **live-verified (post-audit-fix, bc-admin `b20a295`):** Metric Registry UI at `catalog/metrics/directory` — tabbed Family/Group/Member, member provenance detail, governed create doors. Auditor's 5 defects fixed + re-verified: **group create w/ required grain** (Groups 135→136 live), **derived realization** via `members/realized` (frozen `realized_metric_contract_uid` no longer drives state), **contract links → `catalog/metrics/governed/:uid`** (opens the governed detail, not the retired 410), **read-failure states** (no silent zero/absence), **rejection text** (no contract materialized; intake+run retained). Green pinned to PR #45, pending re-review/merge | verified + live UI |
