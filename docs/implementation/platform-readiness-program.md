...
| L4 OC | 🟢 | 🟢 | 🟢 | authoring surface present + wired — the metric-pulled observation-chain console (`POST /contracts/observation-chains`, dry-run + author) authored **on-demand** by design; standalone OC page list-only by design; OC contracts exist (7). 🟢 = **author execution recorded** (legend upgraded from *door present*, 2026-09-20): (a) this-pass **observed dry-run** through the running endpoint (platform_admin) — HTTP 200, `outcome=dry_run`, businessObjectCode `journal_entry`, plan digest `2a61aa56…7048e068`, deterministic across two calls, **zero writes (Inv V)** — plus a fail-closed proof (a stale AC@1.0.0 pin refused "not active"; the active AC@1.1.0 resolved); (b) **3 historical committed author runs** (`runtime.chain_authoring_run`, 2026-08-14) whose OC targets are live. See evidence manifest §Execution records (OC/CC/directory) 2026-09-20. `observation_field_map`=0 = retired legacy resolver table (mappings in OC body) |
| L5 CC | 🟢 | 🟢 | 🟢 | CC-v2 body authored **on-demand** in the same console (demand-pull by design); standalone CC page list-only + dedicated wizards removed (D418) — both by design; CC contracts exist (5, fields+mappings populated). 🟢 = **author execution recorded** (legend upgraded from *door present*, 2026-09-20): the same governed call authors the CC alongside the OC — the this-pass dry-run's normalized plan carries the CC-v2 canonical body (digest `2a61aa56…7048e068`, zero writes) and each of the 3 committed runs produced a **live canonical_contract** (3 CC targets verified live). See evidence manifest §Execution records (OC/CC/directory) 2026-09-20. `canonical_mapping`=0 = retired legacy resolver table |
| L6 MCF | 🟢 | 🟢 | 🟢 | metric_contract 432; `mcf.certification_record` 1316; full |
| L7 Reader | 🟢 | 🟢 | 🟢 | reader_observation_binding populated; full |
| L8 Metric Directory | 🟢 | 🟢 | 🟢 | full **Metric Directory** UI — tabbed Family/Group/Member (counts + coverage + realizability) + member provenance detail + governed create doors (New Family/Group direct; Author metric via the M12 panel with maker/checker/judge provenance). Auditor CHANGES-REQUIRED addressed across two rounds (@ bc-admin `04b796f`): grain-required group create (135→136 live), derived realization state (`members/realized`), governed-route contract links (no 410), rejection text — plus the r2 remainder: **Realized facet keyed on realization association** (`realization_source`), so the 193 `audit_pending` realized members are now under Realized (**246 live**, was 53), lifecycle labels preserved; and **GroupsPane read-failure handling** (a failed groups/family read renders an error, not "No groups"). **Merged — bc-admin PR #45 → `800cdca` (auditor ACCEPTED WITH BOUNDARY).** Create doors **execution-verified** (governed rows, 2026-09-19): New Family (`7fa70579` finance/treasury, theme fx_exposure_and_hedging), New Group (`d8014337` je_line_grain_probe), M12 author panel (`cf6f50ea`, verdict APPROVE_FOR_DRAFT) → M12.5 materialize (draft metric contract `177b865b` supplier_invoice_cycle_time). See evidence manifest §Execution records (OC/CC/directory) 2026-09-20. |
