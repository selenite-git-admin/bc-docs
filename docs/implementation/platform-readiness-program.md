...
| L8 Metric Registry | 🟢 | 🟢 | 🟢 | full **Metric Registry** UI — tabbed Family/Group/Member (counts + coverage + realizability) + member provenance detail + governed create doors (New Family/Group direct; Author metric via the M12 panel with maker/checker/judge provenance). Auditor CHANGES-REQUIRED (5 defects) **fixed + re-verified** @ bc-admin `b20a295`: group-create-with-grain (135→136 live), derived realization state (`members/realized`, not the frozen pointer), governed-route contract links (no 410), read-failure states, corrected rejection text. **Green is pinned to PR #45 (open, held for review) pending its re-review/merge.** |
