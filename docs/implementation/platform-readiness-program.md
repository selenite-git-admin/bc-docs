...
| L4 OC | 🟢 | 🟢 | 🟢 | authoring surface present + wired — the metric-pulled observation-chain console (`POST /contracts/observation-chains`, dry-run + author) authored **on-demand** by design; standalone OC page list-only by design; OC contracts exist (7). 🟢 = *door present* (legend). **Backend author execution recorded** (endpoint + historical, **not** UI, 2026-09-20): (a) this-pass observed **endpoint** dry-run (platform_admin) — HTTP 200, `outcome=dry_run`, businessObjectCode `journal_entry`, plan digest `2a61aa56…7048e068`, deterministic, **zero writes (Inv V)** — plus a fail-closed proof (stale AC@1.0.0 refused; active AC@1.1.0 resolved); (b) **3 historical committed author runs** (`runtime.chain_authoring_run`, 2026-08-14, caller provenance unknown) whose OC targets are live. **A successful author through the current metric-pulled UI remains unverified** (no UI receipt captured). See evidence manifest §Execution records 2026-09-20. `observation_field_map`=0 = retired legacy resolver table (mappings in OC body) |
| L5 CC | 🟢 | 🟢 | 🟢 | CC-v2 body authored **on-demand** in the same console (demand-pull by design); standalone CC page list-only + dedicated wizards removed (D418) — both by design; CC contracts exist (5, fields+mappings populated). 🟢 = *door present* (legend). **Backend author execution recorded** (endpoint + historical, **not** UI): the same governed call authors the CC alongside the OC — the this-pass endpoint dry-run's normalized plan carries the CC-v2 canonical body (digest `2a61aa56…7048e068`, zero writes) and each of the 3 historical committed runs produced a **live canonical_contract** (3 CC targets verified live). **Current-UI author execution unverified.** See evidence manifest §Execution records 2026-09-20. `canonical_mapping`=0 = retired legacy resolver table |
| L6 MCF | 🟢 | 🟢 | 🟢 | metric_contract 432; `mcf.certification_record` 1316; full |
| L7 Reader | 🟢 | 🟢 | 🟢 | reader_observation_binding populated; full |
| L8 Metric Directory | 🟢 | 🟢 | 🟢 | full **Metric Directory** UI — tabbed Family/Group/Member (counts + coverage + realizability) + member provenance detail + governed create doors (New Family/Group direct; Author metric via the M12 panel with maker/checker/judge provenance). Auditor CHANGES-REQUIRED addressed across two rounds (@ bc-admin `04b796f`): grain-required group create (135→136 live), derived realization state (`members/realized`), governed-route contract links (no 410), rejection text — plus the r2 remainder: **Realized facet keyed on realization association** (`realization_source`), so the 193 `audit_pending` realized members are now under Realized (**246 live**, was 53), lifecycle labels preserved; and **GroupsPane read-failure handling** (a failed groups/family read renders an error, not "No groups"). **Merged — bc-admin PR #45 → `800cdca`** (auditor ACCEPTED WITH BOUNDARY; that acceptance did **not** certify live UI execution or counts). Create-door governed rows verified to **exist** (persistence, read-only): New Family (`7fa70579` fx_exposure_and_hedging), New Group (`d8014337` je_line_grain_probe), M12 author panel (`cf6f50ea`, APPROVE_FOR_DRAFT), M12.5 draft contract (`177b865b` supplier_invoice_cycle_time). **Client-path per action:** family/group/M12-panel = operator-reported UI (SES-b20b26); **M12.5 materialization = endpoint-observed**, not UI-authored. Read-only rows prove persistence, not the writing client. See evidence manifest §Execution records 2026-09-20. |
