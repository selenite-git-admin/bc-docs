...
| Quality Gates and Chain Integrity | Operating Model | Owns the gate definitions and verdict semantics. This chapter records that gate verdicts land in `process_audit_finding` and `qa_nc_records` and that the D366 L-node gate consults `contract.l_node_semantic_verdict` *(RETIRED — dropped D481 R3, never populated)* at session close, but the gate's authority over runtime decisions is owned by Quality Gates |
...
| DEC-804874 | D366 L-node semantic gate at session close | Session close consults `contract.l_node_semantic_verdict` *(RETIRED — dropped D481 R3, never populated)* rows on bc-core; on a red verdict, close is blocked unless an override rationale of at least 40 characters is supplied; on override, a `l-node-regression` follow-up task is auto-spawned and the rationale is persisted to the change record. The gate fails open on infrastructure outage |
