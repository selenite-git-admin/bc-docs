...
| `governance/adrs/ADR-1918d0.md` (DEC-1918d0/D162) | governance-ADR | SI-D-1 | partial (owner-gated) | delegates schema inventory to `architecture/database/schema-map.md` which is **absent**; inline "82 tables/12 schemas" is a frozen/stale count (live 20/258). Fix: bc-db owner re-targets the delegation at the live/generated source; annotate the inline count as dated. | 2026-09-21 |
| `CLAUDE.md` (barecount-devhub) | reference / operational-memory | SI-D-1 | partial | June-06 snapshot **freezes** `mcf.metric_contract = 5` (live 433). Fix per the rule: mark June historical + **point to the live source**, do not re-freeze the number. | 2026-09-21 |
