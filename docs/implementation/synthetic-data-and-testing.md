...
The discipline that prevents test runs from contaminating a production tenant's runtime evidence is therefore not invariant suppression. It is tenant isolation: TestBenchModule writes to the addressed tenant database (resolved through the standard `TenantMiddleware` plus continuation-local storage chain), and the design intent is to address a dedicated test tenant (qa-bench) so that production tenants stay clean. The current state has only one provisioned tenant (`sandbox1`); the qa-bench tenant exists in a SQL comment as design intent but is not seeded. Pointing TestBenchModule at any provisioned tenant produces real runtime Evidence in that tenant's database, a current-state risk recorded in the drift inventory below.
...
Volume control is per-profile and per-month with fiscal-period multipliers (month-end +30%, quarter-end +50%, year-end +2x). Integrity checks run via `sdg check <profile>` and cover three categories: referential (foreign-key validation), temporal (date-range bounds), and financial (journal balance per the seven SAP ECC invariants in `src/simulators/sap-ecc/validate.ts`). The integrity surface is part of the as-built generator and is named here so a reader does not assume bc-sdg is a one-way producer with no self-validation.

bc-sdg runs against its own PostgreSQL instance (separate from bc-core's local Postgres) and does not call bc-core; bc-core's readers call bc-sdg, not the other way around. The integration is one-way pull. Master plan PLN-3ff708 (Full-Scale SAP Synthetic Data Generator) anticipated a broader synthetic generator surface; the current state realizes the SAP-side and the cross-system Salesforce/MES correlations but does not yet realize the reference-data sources or any cloud deployment posture. The chapter records this as drift, not as a gap in the as-built description.

### Scenario Flags (opt-in realism)

bc-sdg supports opt-in scenario flags that introduce source-system realism characteristics that would otherwise be absent from the default deterministic output. Scenario flags are off by default so that the default generation remains byte-identical across runs. Each flag enables one specific realism dimension.

| Flag | CLI | Effect |
|---|---|---|
| `fxDivergence` | `--fx-divergence` | Introduces foreign-currency transactions. Approximately 20% of generated documents are denominated in a non-local currency (USD, GBP, JPY). Document currency amounts (`WRBTR`) and local currency amounts (`DMBTR`) diverge by the simulated exchange rate. Per-line `DMBTR` is independently rounded, matching real SAP behavior where line-level local-currency totals may differ from header-level conversion by ±0.01 per document. A `balanceFxDmbtr()` function ensures BSEG debit/credit DMBTR totals balance within each document despite per-line rounding. |

Scenario flags are declared in the `ScenarioFlags` interface (`src/simulators/sap-ecc/generate.ts`) and threaded through `GenerationContext.scenarios`. Adding a new flag follows the same pattern: declare on `ScenarioFlags`, wire through CLI, and guard the realism behavior behind `if (scenarios?.flagName)`.

When `--fx-divergence` is off, all generated rows have `WRBTR === DMBTR` and `WAERS === HWAER === 'EUR'`. This masks real-world issues (currency-mixed sums, FX rounding) that only surface when the flag is enabled.

**Governing source.** `bc-sdg/src/simulators/sap-ecc/generate.ts`; `bc-sdg/src/simulators/sap-ecc/transactions.ts`.
...
qa-bench is a tenant identifier that appears in the test-bench DDL comment as the design-intent test tenant ("Tenant (qa-bench) runs the chain; report collector publishes here"), but the tenant is not provisioned at the time of writing. The active dev environment carries one tenant: `sandbox1`. No `tenant.tenants` row for `qa-bench` exists; no `tbc_qa_bench_dev` database is created; no seed script provisions it.

The implication for test-bench discipline is direct. If a test-bench scenario is created with `tenantSlug = 'sandbox1'` (the only currently-provisioned slug) and execute-chain runs against it, the resulting Source Object, Canonical Object, Metric Snapshot, Evidence, and Lineage rows land in `tbc_sandbox1_dev`. Those rows are indistinguishable from production runtime rows in `sandbox1`'s scope. The chapter records this as an Open drift item: the qa-bench tenant must be provisioned before the test bench can be used safely against any tenant other than a deliberately-disposable one.

A future provisioning of qa-bench would follow the standard tenant onboarding path per `seed-tenant-dbs.ts` (tenant identity row in `tenant.tenants`; per-tenant `tbc_qa_bench_dev` database; seven schemas applied from `docker/redesign/03-tenant-db.sql`; dynamic fact tables created per activated contract by `SchemaProvisionerModule`). The Tenant Onboarding chapter (queued in the Onboarding section) owns the operational procedure; this chapter records only the structural design intent and the current absence.
...
| Repository | Framework | Files | Shape |
|---|---|---|---|
| bc-core | Vitest | 62 unit (`*.spec.ts`) plus 17 integration (`*.integration.spec.ts`); 79 specs total | Unit specs use mocks for fast isolation; integration specs hit real PostgreSQL via `postgres-js`; gated by `BCCORE_INTEGRATION_DB=1` env so unit runs stay fast |
| bc-ai | pytest 8.0+ with pytest-asyncio | 4 files (`test_prompt_loader.py`, `test_pipeline.py`, `test_budget.py` plus `__init__.py`) | Filesystem fixtures via `tmp_path`; `monkeypatch` for env; `unittest.mock.MagicMock` and `patch` decorators; no real DB hits |
| bc-portal | Vitest reportedly configured | 0 files reported | Prior grounding reports test-scaffolding present and zero specs authored; bc-portal repository not readable for independent verification in this drafting pass (see grounding caveat below) |
| bc-admin | None | 0 files | No test framework in `package.json`; no testing-library dependencies |
| barecount-devhub | None for application code; Playwright in devDeps | 0 application specs; Playwright unused | Pre-commit hook validates MCP tool registry shape (`scripts/check-mcp-tools.mjs`) but is not a test runner |
...
| bc-portal has 0 test files (testing-library installed but unused) | Open | The frontend testing scaffolding is present but no specs are authored |
...
| bc-ai pytest suite is mock-only (4 files) | Low | No real-DB integration spec; the maker-checker-gate triplets are exercised via mocks; cross-family verification has no automated regression test |
| End-to-end and cross-service tests are absent | Open | The full bc-portal -> bc-core -> bc-ai -> DevHub -> tenant DB chain is not exercised by automated tests; manual exercise is the current substrate |
...
| Synthetic data and runtime data share the same fact tables in the addressed tenant | Open | A consequence of the no-synthetic-mode design. If the test bench writes to a tenant that also receives production runtime, the two streams cannot be distinguished without an external marker (tag column, scenario reference, or per-row provenance). No such marker exists today |
| bc-sdg default output masks currency and sign realism | Low | Without `--fx-divergence`, all amounts are single-currency (`EUR`) with `WRBTR === DMBTR`, hiding FX rounding issues. Without sign correction at the canonical boundary (TSK-04e6df), credit-side amounts silently overstate sums. Both issues are invisible until the respective realism flag / OC sign_indicator pattern is active |
...
| DEC-bebaec | Chain Status SSOT | The TestBench preflight currently consults `IntegrityService`; migration to the chain-status SSOT (`ChainStatusService`) is queued. The current path is recorded as drift, not as the canonical preflight authority |
