...
For AWS Shared, AWS Separate, and BYO-DB variants, the tenant database is provisioned in this step. The governed entry point is the **`POST /tenants`** API (`TenantProvisioningApiService`, bc-core PR #727, merged): a fail-closed DB-per-tenant flow that wraps the provisioner (`bc-core/src/registry/seed/seed-tenant-dbs.ts` → `docker/redesign/03-tenant-db.sql`). It validates the tenant slug (a SQL-identity gate), provisions `tbc_<slug>_<env>`, and — before marking the tenant `active` — verifies the post-condition contract: all six tenant schemas (including `progression`), the complete baseline relation manifest, and the D575 evidence append-only triggers bound exactly to their relations. It records append-only provisioning evidence, resumes an interrupted provision idempotently, and preserves suspended/retired lifecycle states. The batch/bootstrap CLI runs the same underlying provisioner. BYO-DB provisioning attaches the customer's database via a connection string recorded in the Onboarding Record. (History: `POST /tenants` was previously disabled fail-closed under D575 because the retired schema-per-tenant `t_<slug>` layout it built was runtime-unusable and outside evidence-immutability enforcement; PR #727 restored it DB-per-tenant.)
