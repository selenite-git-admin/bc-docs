---
uid: DEC-af8247
title: "Cross-domain metric scope and tenant applicability policy"
description: "Under the MC Envelope Governance authority established by DEC-889238 (C2), declares industry-applicability metadata as a first-class property of both MC envelopes and tenants. Per-MC applicable_industries from a 15-value closed enum (cross_industry, hospitality, manufacturing, retail, banking, insurance, healthcare, energy, telecommunications, software_saas, professional_services, real_estate, transportation, pharmaceuticals, media_entertainment, agriculture). Per-tenant tenant.industries from same enum. Compatibility rule: cross_industry OR non-empty intersection passes. Gate fires at tenant-binding create and as a filter on binding-candidates read. Existing bindings grandfathered. Per-tenant compat-mode (shadow/block) flippable independently per authority — sda_preflight_mode and mc_scope_mode are separate switches. Names DBCPs 1h/1i/1j for the tenant.industries, metric_definition.applicable_industries, and master.industry schema additions."
status: decided
date: 2026-05-12T08:46:39.020Z
project: platform
domain: contracts
subdomain: mc-envelope
focus: tenant-applicability
---

# Cross-domain metric scope and tenant applicability policy

## Context

Apex Pool 1 audit surfaced at least four cross-domain leaks among producing MCs: mc__goppar (hospitality - Gross Operating Profit Per Available Room), mc__gross_written_premium_gwp (insurance), mc__insurance_revenue_ifrs_17 (insurance), mc__value_of_new_business_vnb (insurance). Apex operates in software_saas — none of these MCs is meaningful for the tenant. The function/subfunction taxonomy is too coarse to catch this since every tenant has a finance function. The policy adds industry as a first-class metadata dimension at MC and tenant levels with deterministic intersection-based admissibility at tenant binding.
...
7. **Independently addressable mode switches** — C3 and SDA Phase 2 ship and roll out independently per §7.
...
- Authority: MC Envelope Governance (peer to SDA, established by DEC-889238 / C2).
...
- Compat mode: per-tenant shadow/block, **independently addressable per authority** (`sda_preflight_mode` and `mc_scope_mode` are separate switches).
...
- **DEC-a17d0f** (D403) — SDA umbrella ADR. C3 shares the compat-mode mechanism design pattern with SDA Phase 2 but the switches are independently addressable per authority.
- **DEC-b7affa** (D404) — C1 amendment adding G11 to SDA. Sibling Lane C decision.
- **DEC-889238** (D405) — C2 establishing MC Envelope Governance as a peer authority. **C3 lives under this authority as a sibling decision.**
...
- **Evidence MWRs (in bc-docs-v3/docs/onboarding/metric-work-records/_cross/):**
  - `2026-05-12-pool1-trust-audit-46-producing-SES-594568.md` — `mc__goppar`, `mc__gross_written_premium_gwp`, `mc__insurance_revenue_ifrs_17`, `mc__value_of_new_business_vnb` observed in Apex producing surface.
  - `2026-05-12-apex-phase0-readiness-walkthrough-SES-594568.md` §A4 — cross-domain CFs flagged as governance scope #3.
