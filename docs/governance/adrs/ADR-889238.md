---
uid: DEC-889238
title: "MC Envelope Governance — deduplication policy"
description: "Establishes MC Envelope Governance as a peer authority to SDA (DEC-a17d0f). Defines the duplicate-MC fingerprint as the six-tuple (canonical formula + input CF set + output CF set + constant CF set + grain CF + temporal pattern). Subfunction is explicitly NOT part of the fingerprint. Gate fires at MC envelope create and version activate; default older-created-at survivor with D366-style override path; non-survivor marked superseded (not withdrawn) preserving historical bindings and snapshots per Foundation Invariant III. Names mc_envelope_audit_record audit ledger as future DBCP. Phase 1 service scope: preflight check + supersede endpoint + retroactive /duplicates projection + audit ledger. Full MC envelope lifecycle deferred to a future ADR."
status: decided
date: 2026-05-12T08:45:28.821Z
project: platform
domain: contracts
...
# MC Envelope Governance — deduplication policy

## Context

Three duplicate MC pairs surfaced in the Apex Pool 1 trust audit (mc__roa_return_on_assets / mc__return_on_assets_roa; mc__net_working_capital_ratio / mc__working_capital_as_of_revenue; suspected mc__fixed_asset_turnover / mc__fixed_asset_turnover_ratio). No authority today prevents duplicate MC registration; the Apex audit also showed 14 producing MCs sharing a single mapping fingerprint via cc__actual_ledger.actual_ledger_amount sum. Envelope-layer funnel-padding is the analogue of cc_field_mapping G10 collision one layer up. SDA is the wrong place to govern envelopes (different lifecycle, different scope); a peer authority keeps separation clean.
...
| **MC Envelope Governance** (this ADR) | MC envelopes — uniqueness, lifecycle (future scope), structural integrity (formula↔variables↔grain consistency); deduplication; cross-domain scope (C3 — sibling ADR) |
...
- Cross-domain scope (C3 — sibling ADR under the same MC Envelope Governance authority).

## 6. References

- **DEC-a17d0f** (D403) — SDA umbrella ADR. C2's peer authority sits alongside it.
- **DEC-b7affa** (D404) — C1 amendment to DEC-a17d0f adding G11. Sibling Lane C decision; shares the override-mechanic pattern.
...
- **Evidence MWRs (in bc-docs-v3/docs/onboarding/metric-work-records/_cross/):**
  - `2026-05-12-pool1-trust-audit-46-producing-SES-594568.md` — 3 observed duplicate pairs.
  - `2026-05-12-apex-phase0-readiness-walkthrough-SES-594568.md` §A4 — MC envelope duplicates flagged as governance scope #2.
