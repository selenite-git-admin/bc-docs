...
## Amendment (DEC-0d5b39, 2026-08-18)
The **exact-selection->Lineage clause** is amended: the `as_of_period_end` "latest accepted upstream Snapshot at/before P" selection must record the **exact consumed Metric Snapshot IDs** in persisted Lineage (`input_references_json`), not upstream Metric Contract UIDs (FND-R2-1, Invariants IV/VI). **Retained:** composite routing, formula execution, fail-closed missing-input behavior, and governed persistence (`DEC-5ea578`, cite/affirm). Status remains `decided`.
