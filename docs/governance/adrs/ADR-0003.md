---
uid: DEC-000003
title: "Three-contract model — Raw, GDP, KPI"
description: "Original three-contract taxonomy: Raw (ingestion), GDP (transformation), KPI (metrics). Superseded by six-family contract model."
status: superseded
date: 2025-09-17
project: platform
domain: contracts
...
<!-- SUPERSEDED: This ADR predates the current contract model. The three-contract model
     (Raw, GDP, KPI) has been superseded by the seven-contract-type model (D018) and
     three-chain architecture (D022). Retained for historical context only.
     Vocabulary in this document does not comply with Foundation governance. -->
...
## Context
DetaJetty governs data with explicit contracts rather than ad-hoc pipelines. We need a clear, durable model that separates concerns across ingestion, normalization, and business metrics—while enabling enforcement, lineage, and audit.
