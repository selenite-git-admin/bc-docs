...
## Amendment 2 — activation numeric boundary (B-03 decision, operator-approved 2026-07-11)

The external audit gate is implemented as **(b) then (a)** (Codex coordinated-v3 review V3-01, operator-approved):

- **(b) now:** land the `audit_pending → active` gate but permit activation under the current binary64 runtime ONLY for a **machine-proven exact** subset. Exactness is decided by a static, versioned proof (`mcf-exactness-v1`) over the formula AST + declared input domains — never a label or operator judgment. `EXACT` = `count`; integer-bounded `plus/minus/multiply` whose complete result interval stays within ±(2^53−1) by interval arithmetic over declared bounds; and nonfractional classification/date outputs with no fractional numeric computation. Everything else — currency, ratios, percentages, averages, percentiles, weighted calculations, `divide`/`mod`, fractional literals, and any unbounded numeric input — proves `NOT_PROVEN` and remains non-active (`audit_pending`/`audit_blocked`). Failure to prove exactness is **rejection, not override**. The proof result + `mcf-exactness` algorithm version + input bounds + AST hash are inputs to `mcf-package-v3`.
- **(a) mandatory next:** deterministic end-to-end decimal execution (canonical decimal transport, DB-decode, shared verifier+runtime implementation and vectors) is the required correctness program before ANY decimal-dependent (financial) metric activates. Option (b) is rollout sequencing and is explicitly NOT a claim that financial metrics are calculator-grade.

`mcf-package-v3` binds the resolved evaluation-policy digest, the evaluation-policy rulebook version+digest, the execution-engine version, the numeric-representation version, the exact grain Entity Version, and the `mcf-exactness-v1` proof — so a metric's audited identity fixes exactly how (and in what numeric regime) it is computed. Full design: `barecount-devhub/artifacts/metric-audit/DBCP-coordinated-v3-tracks-abc-2026-07-11.md` + `DBCP-coordinated-v3-amendment-2026-07-11.md`.
