...
Surfaced as governance scope gap #1 in the Apex Phase 0 readiness walkthrough (2026-05-12) and quantified by the Pool 1 trust audit: 14 of 46 currently-producing Apex MCs map measure-currency CFs through fiscal-year (GJAHR) or exchange-rate (KURSF) BFs without any G1-G10 objection. G11 closes this gap with deterministic, mostly non-overridable rejection at cc_field_mapping authoring time. Matrix validated against 246 Apex variable inputs (160 MCs scanned across the Pool 1 audit + Phase 0 walkthrough candidate set); 35+ direct evidence hits across the four operator-named non-overridable classes (measure↔temporal/dimension; amount↔fiscal-year/date/period; exchange-rate-as-cost; ratio/percent/score used as raw sum amount). Matrix-review note (2026-05-12-c1-matrix-review-against-apex-SES-594568) and three approved refinements (M1 count_* tightening; M2 compute__derived skip; M3 categorical-rating reclassification to code) are incorporated.
...
### 2.5 Categorical ratings are `code`, not `measure-score`
...
| **`measure-score`** | (measure-score, latest) accept; (measure-score, sum) review *(summing scores is unusual)*. **Reserved for true numeric scores** (NPS 0-10, FICO 300-850 etc.). **Categorical ratings** (HIGH/MEDIUM/LOW etc.) are classified as `code` family with `code_vocabulary_code` per G11b, **not** as `measure-score` — see §2.5 |
...
|---|---|---|---|
| This amendment | ADR amendment to DEC-a17d0f | Filed | Decided |
| DBCP-1a + 1b + 1c + 1f | Phase 1 substrate | Drafts | Operator approval pending |
| DBCP-1g (BF.semantic_family column) | New schema | Named here, not authored | Future |
| BF certification flow (G5 extended to BFs) | Phase 1 service code | Pending Phase 1 start | Future |
| G11 gate code | Phase 1 service code | Pending Phase 1 start | Future |
| G11b code-vocabulary sub-gate | Future amendment | Deferred | Future |
| Phase 2 preflight integration (G9 / G10 / G11) | Phase 2 service code | Per DEC-a17d0f §9 Phase 2 | Future |
...
- **Anchor:** DEC-a17d0f (D403) — Semantic Definitions Authority. This amendment **does not supersede** DEC-a17d0f; it **adds** G11 to its gate set and extends G5 to BFs.
- **Closed enum source:** DEC-804874 (D366) — L-Node Semantic Gate. semantic_family closed enum, 24 values, hyphenated kebab-style.
- **Naming standard:** DEC-69f09e (D148) — ISO 11179. P5 hyphenation divergence carried forward.
- **Canonical Field primitive:** DEC-d72560 (D301, superseded by DEC-a17d0f on lifecycle) — cc_field_mapping schema, 5 resolution rules.
- **Evidence MWRs (in bc-docs-v3/docs/onboarding/metric-work-records/_cross/):**
  - `2026-05-12-apex-phase0-readiness-walkthrough-SES-594568.md` — first surfaced the BF-CF compatibility gap as governance scope #1.
  - `2026-05-12-pool1-trust-audit-46-producing-SES-594568.md` — quantified 14/46 producing Apex MCs as fiscal-year-bound; provided Cases A, B, C examples.
  - `2026-05-12-c1-matrix-review-against-apex-SES-594568.md` — validated the §4 matrix against 246 Apex variable inputs; surfaced refinements M1-M3 which are incorporated above.
  - `2026-05-12-phase1-dbcp-drafts-1a-1b-1c-1f-SES-594568.md` — DBCP-1g named here sits after this draft sequence.
- **Foundation:** bc-docs-v3/docs/foundation/the-invariants.md (Invariant I — Meaning is evaluated once; G11 is the type-shape complement to G10's identity-shape Meaning-once invariant).
