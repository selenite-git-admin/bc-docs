...
- Its operands carry acyclic d467 composite bindings to two **leaf** metrics: `ar_balance` (mcv `61a876e7`) = **`active`**, and **`gross_invoiced_amount` (mcv `8a38e79c`) = `audit_pending`** — its **only extant** version, with **`is_current=false`**. The runtime composite loader (`CompositeMetricEvaluationService.loadCompositeSpec`, bc-core `53bb1115`, `composite-metric-evaluation.service.ts:353`) resolves each upstream operand by `bound_metric_contract_uid` + `is_current=true` + `governance_state_code='active'` (a LEFT JOIN) — **not** by a stored operand version id. So the gross leaf currently resolves to **no active/current upstream version**. (Cert history: DSO approved→active 2026-06-30; the gross leaf active→audit_pending reintake 2026-08-02.)
- Therefore, **at the metric layer**, DSO has no evaluable composite because a required leaf has no active/current version. This is a **necessary** blocker; it does **not** by itself prove there is no *additional* downstream defect (source/admission/CO/OC/projection) — those are **not** verified here.

**Corrected route (a).** Certifying + activating the `gross_invoiced_amount` leaf (`audit_pending → active`) through the governed MCF certification path is a **necessary candidate repair** — it removes the verified metric-layer blocker. It is **not proven sufficient**: whether DSO then produces a real snapshot for Kaveri additionally requires the tenant source/admission/resolution chain and remains to be verified end-to-end (no full leaf-chain audit is claimed here). This stays "route (a)" as the *leanest* candidate (activate the one leaf DSO needs) versus route (b) (corpus-wide family remediation, parked TSK-afd7ff). The prior "OC re-pin / operand-projection gap" framing is withdrawn as unsupported by the registry facts, but a downstream OC/projection defect is not thereby excluded — it is simply unverified.
