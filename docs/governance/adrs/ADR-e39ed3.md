...
Not every AWS resource has a settable physical name, not every non-nameable one is taggable, and some are named by a separate contract — so a single "every physical name follows the format" rule is impossible. The registry (§3 file) partitions **every governed resource type into exactly one of four classes**, and the Aspect fails synth on any encountered type that is in none (forcing a registry amendment — the classification is complete by construction):
...
- **Class C — non-nameable and non-taggable** (`classC_nonNameableNonTaggable`): structural glue with neither a name property nor tag support — so neither the name check nor the tag check applies: `AWS::EC2::Route`, `AWS::EC2::VPCGatewayAttachment`, `AWS::EC2::SubnetRouteTableAssociation`, `AWS::IAM::Policy` (inline), `AWS::EC2::SecurityGroupIngress`, `AWS::SecretsManager::SecretTargetAttachment`, `AWS::CDK::Metadata` (tool-injected). Identity is the CDK construct path / CFN logical id; the Aspect asserts neither a physical name nor tags for these.
- **Class D — contract-named + tag-enforced** (`classD_contractNamed`): taggable resources whose name follows a **separate contract**, not the `{scope}-…` convention: `AWS::SSM::Parameter` (named by the SSM path contract `/bc/{stage}/{domain}/…`). The **required tags ARE enforced**, and the name property is validated against the contract's regex (from the registry `patterns`), not the convention. (Classes C and D were completed during first rollout as real stacks were synthesized; additive only.)
