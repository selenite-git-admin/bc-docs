...
| Schedule cadence | **Target:** per-MC `temporal_gate.schedule.cron` (realtime → monthly per the cron preset). **As-built:** the mounted scheduler runs **one nightly full-catalog campaign**; it does not read a per-MC cron schedule. Per-MC scheduling is DEC-01bd6b target doctrine |
| Idempotency | **Retry idempotency (as-built):** a retry of the **same deterministic `run_id`** re-lands the same row. A **new invocation** receives a **new run/evaluation identity** even for unchanged inputs, and a **changed** value yields a **new** Metric Snapshot version — the prior row is never mutated or reused, and is superseded **on read** (Invariant III; matching the Runtime Operations run-lifecycle rule). Same-run retry idempotency is not a new forward evaluation act |
